...
$photo_query=sprintf("SELECT * FROM %s WHERE 1 %s %s ORDER BY `gameDate`",ITEM_TABLE,$yearString,$nightString);	
//echo $photo_query;
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>   
                    ...
                    <tr>
                     ...
                     <div class="selectItem">
                       <select name="gameYear" id="gameYear" onChange="window.location =this.options[this.selectedIndex].value">
                         <option value="manageGames.php?gameYear=All" <?php if ($_SESSION['sGameYear'] == 'All') echo "selected=\"selected\"" ?>>Show All</option>      
                         <?php 
                      for ($y=2012; $y <= (date('Y')+2); $y++) { ?>
                         <option value="manageGames.php?gameYear=<?=$y?>" <?php if ($_SESSION['sGameYear'] == $y) echo "selected=\"selected\"" ?>><?=$y?></option>
                         <?php } ?>
                         </select>                          
                       </div>
                     ...
                     <div class="selectItem">
                       <select name="gameNight" id="gameNight" onChange="window.location =this.options[this.selectedIndex].value">
                         <option value="manageGames.php?gameNight=All" <?php if ($_SESSION['sGameNight'] == 'All') echo "selected=\"selected\"" ?>>Show All</option>      
                         <?php 
                      foreach ($nightArray as $night) { ?>
                         <option value="manageGames.php?gameNight=<?=$night?>" <?php if ($_SESSION['sGameNight'] == $night) echo "selected=\"selected\"" ?>><?=$night?></option>
                         <?php } ?>
                         </select>                          
                       </div>
                       </td>
	 ...
	 <tr>
	   ...
	   <td class="adminTableItemHeading" width="15%" align="left">Team A</td>
	   <td class="adminTableItemHeading" width="16%" align="left">Team B</td>
	   <td class="adminTableItemHeading" width="12%" align="left">Court</td>
	   <td width="8%" class="adminTableItemHeading"  align="center">Edit</td>
	...
	<tr <?php if ($color_flag == 1) { echo ('class = "background_lite"'); } else { echo ('class = "background_dark"'); } ?>>
	  ...
	  <td align="left" valign="top"  class="tableContent" ><?=stripslashes($teamArray[$_SESSION['sGameYear']][$row_photo_set['gameTeamA']]['teamName'])?></td>
	  <td align="left" valign="top"  class="tableContent" ><?=stripslashes($teamArray[$_SESSION['sGameYear']][$row_photo_set['gameTeamB']]['teamName'])?></td>
	  <td align="left" valign="top"  class="tableContent" ><?=stripslashes($row_photo_set['gameCourt'])?></td>
